...
df_enrichment <- subset(df_enrichment, adjp<0.05)
...
if(eTerm$g$ontology %in% "AA"){
	...
	vec_outfile <- sapply(1:nrow(df_enrichment), function(i){
		query <- rownames(df_enrichment)[i]
		out.file <- paste0(tempfile(), ".", query, '.graphml')
		xGraphML2A2(data=data, query=query, node.label="label", node.color="lfc", colormap="white-orange", filename=out.file, RData.location=RData.location, verbose=F)
		out.file
	})
