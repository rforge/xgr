...
**Extract genes that are significantly induced by IFN compared to the naive state**
...
The first 5 rows of the data frame `df` are shown below, with the column `logFC_INF24_Naive` telling the log2-transformed fold change of `naive` expression divided by `IFN` expression (thus the induced genes with negative values).

```{r, eval=T, echo=F}
knitr::kable(df[1:5,], caption="", row.names=F)
```
...
```{r, eval=T, echo=F}
obo <- list()
obo[['DO']] <- c('Diseases', 'Disease Ontology', 'DO')
obo[['GOMF']] <- c('Functions', 'Gene Ontology Molecular Function', 'GOMF')
obo[['GOBP']] <- c('Functions', 'Gene Ontology Biological Process', 'GOBP')
obo[['GOCC']] <- c('Functions', 'Gene Ontology Cellular Component', 'GOCC')
obo[['HPPA']] <- c('Phenotypes', 'Human Phenotype Phenotypic Abnormality', 'HPPA')
obo[['HPMI']] <- c('Phenotypes', 'Human Phenotype Mode of Inheritance', 'HPMI')
obo[['HPCM']] <- c('Phenotypes', 'Human Phenotype Clinical Modifier', 'HPCM')
obo[['HPMA']] <- c('Phenotypes', 'Human Phenotype Mortality Aging', 'HPMA')
obo[['MP']] <- c('Phenotypes', 'Mammalian/Mouse Phenotype', 'MP')
...
colnames(df_obo) <- c('Category', 'Name', 'Code')
knitr::kable(df_obo, caption="", row.names=F)
```

Optionally, the test background can be provided by the user. By default, all annotatable genes will be used as the test background. In this case, genes under differential expression analysis will be used as the background.
...
data <- df$Symbol
eTerm <- xEnricherGenes(data=data, background=background, ontology="MsigdbC2CPall", RData.location=RData.location)
...
```{r, eval=T, cache=T, results="hide"}
data <- df$Symbol
eTerm <- xEnricherGenes(data=data, background=background, ontology="DO", RData.location=RData.location)
# view enrichment results for the top significant terms
xEnrichViewer(eTerm)
...
```{r, eval=T, cache=T, results="hide"}
data <- df$Symbol
eTerm <- xEnricherGenes(data=data, background=background, ontology="MP", RData.location=RData.location)
# view enrichment results for the top significant terms
xEnrichViewer(eTerm)
...
net[['PCommonsUN_high']] <- c('Pathways Commons', 'Physical/undirect interactions (with references and >=2 sources)', 'PCommonsUN_high')
net[['PCommonsUN_medium']] <- c('Pathways Commons', 'Physical/undirect interactions (with references and >=1 sources)', 'PCommonsUN_medium')
net[['PCommonsDN_high']] <- c('Pathways Commons', 'Pathway/direct interactions (with references and >=2 sources)', 'PCommonsDN_high')
net[['PCommonsDN_medium']] <- c('Pathways Commons', 'Pathway/direct interactions (with references and >=1 sources)', 'PCommonsUN_medium')
df_net <- do.call(rbind, net)
colnames(df_net) <- c('Category', 'Name', 'Code')
knitr::kable(df_net, caption="", row.names=F)
```

For the pathway-merged direct interactions, the user can also choose network from the individual sources:

```{r, eval=T, echo=F}
net <- list()
net[['PCommonsDN_Reactome']] <- c('Pathways Commons', 'Pathway/direct interactions (only sourced from Reactome)', 'PCommonsDN_Reactome')
net[['PCommonsDN_KEGG']] <- c('Pathways Commons', 'Pathway/direct interactions (only sourced from KEGG)', 'PCommonsDN_KEGG')
net[['PCommonsDN_HumanCyc']] <- c('Pathways Commons', 'Pathway/direct interactions (only sourced from HumanCyc)', 'PCommonsDN_HumanCyc')
net[['PCommonsDN_PID']] <- c('Pathways Commons', 'Pathway/direct interactions (only sourced from PID)', 'PCommonsDN_PID')
net[['PCommonsDN_PANTHER']] <- c('Pathways Commons', 'Pathway/direct interactions (only sourced from PANTHER)', 'PCommonsDN_PANTHER')
net[['PCommonsDN_ReconX']] <- c('Pathways Commons', 'Pathway/direct interactions (only sourced from ReconX)', 'PCommonsDN_ReconX')
net[['PCommonsDN_PhosphoSite']] <- c('Pathways Commons', 'Pathway/direct interactions (only sourced from PhosphoSite)', 'PCommonsDN_PhosphoSite')
net[['PCommonsDN_CTD']] <- c('Pathways Commons', 'Pathway/direct interactions (only sourced from CTD)', 'PCommonsDN_CTD')
df_net <- do.call(rbind, net)
...
subnet <- xSubneterGenes(data=data, network="STRING_high", subnet.size=75, RData.location=RData.location)
```
...
```{r, eval=T, cache=T, results="hide"}
data <- df_cis$variant
eTerm <- xEnricherSNPs(data=data, ontology="EF", RData.location=RData.location)
# view enrichment results for the top significant terms
xEnrichViewer(eTerm)
...
```{r, eval=T, cache=T, results="hide"}
data <- df_cis$variant
eTerm <- xEnricherSNPs(data=data, ontology="EF", include.LD="EUR", LD.r2=0.8, RData.location=RData.location)
# view enrichment results for the top significant terms
xEnrichViewer(eTerm)
...
subnet_SNP <- xSubneterSNPs(data=data, network="STRING_high", distance.max=200000, seed.genes=T, subnet.significance=1e-1, subnet.size=60, RData.location=RData.location)
```
...
In the following subsections, we focus on two inflammatory diseases, Crohn's Disease (CRO) and Celiac Disease (CEL), identifying putative gene networks that are likely modulated by their corresponding lead (or LD) SNPs. The applications in the other diseases can be similarly done.
...
subnet_CRO <- xSubneterSNPs(data=data, network="STRING_high", distance.max=500000, seed.genes=T, subnet.size=30, RData.location=RData.location)
```
...
# find maximum-scoring gene subnetwork with the desired node number=30
subnet_CEL <- xSubneterSNPs(data=data, network="STRING_high", distance.max=500000, seed.genes=T, subnet.size=30, RData.location=RData.location)
```
