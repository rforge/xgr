    ...
    if(length(include.eQTL) > 0){
		...
		res_list <- lapply(include.eQTL, function(x){
			...
			if(sum(grep("JKscience_TS2A",x,perl=TRUE)) > 0){
				...
				# cis-eQTL
				cis <- xRDataLoader(RData.customised='Osteoblast_eQTL', RData.location=RData.location, verbose=verbose)
