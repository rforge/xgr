...
#' @param layout the visual layout. It can be "rectangular" or circular stype ("fan" and "partition")
...
#' @param width the width/height of embedded ggplot obejcts
#' @return 
#' a ggplot2 object
#' @note none
#' @export
...
#' gp <- xSHtree(sMap, tree, layout="rectangular", width=1)
#' gp <- xSHtree(sMap, tree, layout="fan", width=0.4)
#' gp <- xSHtree(sMap, tree, layout="partition", width=0.7)
#' }

xSHtree <- function(sMap, phylo=NULL, layout=c("rectangular","fan","partition"), colormap="spectral", zlim=NULL, tip.size=10, width=1)
{
    ...
    ls_gp <-lapply(1:length(cnames), function(i){
    	xSH(sMap, customised.comp=codebook[,i], colormap=colormap, zlim=zlim) + labs(title=cnames[i]) + theme(plot.title=element_text(hjust=0.5,size=tip.size), plot.margin=unit(rep(0,4),rep("lines",4)))
	...
	if(layout=='rectangular'){
		...
		gp <- gp + ggimage::geom_subview(data=df, aes(x=x, y=y, subview=ls_gp), width=width, height=width) + scale_x_continuous(position="bottom", limits=c(0,max(df$x)))

		data <- as.data.frame(gp$data %>% dplyr::filter(!isTip))
		data$label <- as.numeric(data$label)
		gp <- gp + geom_point(data=data, aes(x=x,y=y,size=label), shape=21, color='grey', fill="white") + geom_text(data=data, aes(label=label,size=label/3))
		...
		if(layout=="fan"){
			...
			gp <- gp + ggraph::geom_edge_diagonal(aes(alpha=1 - ..index..), color='black') + ggraph::geom_node_point(aes(filter=!leaf), color='grey') 
			#gp <- gp + ggraph::geom_node_text(aes(filter=leaf, label=name))
			gp <- gp + theme_void() + coord_fixed() + theme(legend.position="none",plot.margin=unit(c(0,0,0,0),"cm")) + expand_limits(x=c(-1.2, 1.2), y=c(-1.2, 1.2))
			
		}else if(layout=="partition"){
			
			gp <- ggraph::ggraph(ig, layout='partition', circular=TRUE)
			gp <- gp + ggraph::geom_edge_diagonal(aes(alpha=1 - ..index..), color='black') + ggraph::geom_node_point(aes(filter=!leaf,alpha=1 - depth), color='black')
			#gp <- gp + ggraph::geom_node_text(aes(filter=leaf, label=name))
		...
		gp <- gp + ggimage::geom_subview(data=df, aes(x=x, y=y, subview=ls_gp), width=width, height=width)
