...
#'
#' \code{xEnrichHeatmap} is supposed to visualise enrichment results using heatmap in which rows for terms and columns for its members. It returns an object of class "ggplot". 
#'
...
#' gp_heatmap <- xEnrichHeatmap(eTerm)
#' 
#' # 2) PSG
#' eTerm <- xEnricherGenes(data, ontology="PSG", size.range=c(1,20000), min.overlap=0, RData.location=RData.location)
#' gp_heatmap <- xEnrichHeatmap(eTerm, sortBy="none", top_num="auto", FDR.cutoff=1)
#' 
#' # 3) save into the file "xEnrichHeatmap.pdf"
#' mat <- xSparseMatrix(gp_heatmap$data)
#' pdf("xEnrichHeatmap.pdf", width=2+ncol(mat)*0.075, height=2+nrow(mat)*0.1, compress=T)
#' print(gp_heatmap)
#' dev.off()
#' }

xEnrichHeatmap <- function(eTerm, sortBy=c("or","adjp","fdr","pvalue","zscore","fc","nAnno","nOverlap","none"), top_num=10, FDR.cutoff=0.05, CI.one=T, colormap="skyblue-darkblue", verbose=T)
{
    ...
    if(class(eTerm)=='eTerm'){
		...
		if(!is.null(df_enrichment)){
			...
			df_enrichment$label <- paste0(df_enrichment$name, "\n[OR=", df_enrichment$or, ", P=", df_enrichment$pvalue, ", FDR=", df_enrichment$adjp, ", n=", df_enrichment$nOverlap, "]")
