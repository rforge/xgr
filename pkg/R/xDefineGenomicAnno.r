...
#' \itemize{
#'  \item{\code{EpigenomeAtlas_DNaseNarrow}: a list (39 cell types) of GenomicRanges objects; each is an GR object containing narrow Peaks in FDR 0.01 hotspots.}
#'  \item{\code{EpigenomeAtlas_DNaseBroad}: a list (39 cell types) of GenomicRanges objects; each is an GR object containing broad Peaks in FDR 0.01 hotspots.}
#' }
    ...
    if(is(GR.annotation,"GenomicRangesList")){
		...
		if(is(GR.annotation,"list")){
			...
			if(!is.na(GR.annotation)){
				lgr <- xRDataLoader(RData.customised=GR.annotation, verbose=verbose, RData.location=RData.location, guid=guid)
			}
		}
