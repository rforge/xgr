...
#'
#' \code{xGGtree} is supposed to visualise enrichment results using a forest plot. A point is colored by the significance level, and a horizontal line for the 95% confidence interval (CI) of odds ratio (OR; the wider the CI, the less reliable). It returns an object of class "ggplot".
#'
#' @param phylo an object of class "phylo"
#' @param membership a named input vector containing the membership for tips. For this named vector, the element names are clusters, the element values for tip labels
...
#' @return an object of class "ggplot" appended with a phylo object "tree" (appended with a "cluster" list)
#' @note none
#' @export
#' @seealso \code{\link{xGGtree}}
#' @include xGGtree.r
#' @examples
#' \dontrun{
#' # Load the library
...
xGGtree <- function(phylo, membership, layout=c("rectangular","fan"), colormap="ggplot2", extendto=NULL, offset=NULL, open.angle=0)
{
    ...
    if(!(is.vector(membership) & !is.null(names(membership)))){
	...
	# append 'tree'
	tree$cluster <- list(color=col_clusters, mrca=mrca_clusters)
	gp$tree <- tree
