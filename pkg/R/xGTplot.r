...
#'
#' \code{xGTplot} is supposed to draw heatmap on tree tips.
#'
...
xGTplot <- function(gp, data, type=c("bar","ridge"), combined=T, colormap="spectral", ncolors=64, y.text.size=2)
{
	...
	if(type=="bar"){
		if(combined){
			...
			gp2 <- ggplot(df) + geom_segment(aes(x=0, y=y, xend=value, yend=y, color=value), size=1) + scale_color_gradientn(colors=xColormap(colormap)(ncolors), guide=F) + scale_y_continuous(breaks=1:nrow(df_tips),labels=rev(df_tips$label),limits=c(1,nrow(df_tips)),expand=c(0,0.5)) + theme_bw() + theme(legend.position="none",axis.title.y=element_blank(), axis.title.x=element_blank(),panel.grid.major=element_blank(), panel.grid.minor=element_blank())
		}
