#' Function to add coordinates according to communities into a graph
#'
#' \code{xAddCoords} is supposed to add coordinates according to communities into a graph.
#'
#' @param g an object of class "igraph" (or "graphNEL") for a graph with a 'community' node attribute
...
#' ig <- xAddCoords(g, edge.color.alternative=c("grey50","grey95"))
...
#' gp + scale_colour_gradientn(colors=xColormap('jet')(64),limits=c(1,8),breaks=seq(1,8)) + guides(color=guide_legend(title="Community"))
#' }

xAddCoords <- function(g, seed=825, glayout=layout_with_kk, edge.color.alternative=c("grey70","grey95"), verbose=TRUE)
{
	...
	if(is.null(V(ig)$community)){
		stop("The igraph object must have a 'community' vertex attribute.\n")
	}
	...
	if(all(c("type","community","contribution") %in% igraph::vertex_attr_names(ig))){
		...
		df_nodes <- df_nodes %>% dplyr::arrange(community)
	}
