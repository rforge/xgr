...
#' @param identity the entity of input data. It can be either "Symbol" or "GeneID"
#' @param p.type the type of the p-value calcualted. It can be 'nominal' for nominal p-value (calculated using two-sample Wilcoxon rank sum tests) or 'empirical' for empirical p-value (based on the test statistic 'Gene Evolutionary Age Cumulative Ranking')
...
#' res <- xAge(data, p.type="nominal", plot=TRUE, RData.location=RData.location)
#' res$ls_gp_violin[[1]]
#' # based on the test statistic 'Gene Evolutionary Age Cumulative Ranking'
#' res <- xAge(data, p.type="empirical", nperm=10000, plot=TRUE, RData.location=RData.location)
#' res$ls_gp_violin[[1]]
#' res$ls_gp_pdf[[1]]
#'
#' # Genetic regulators 
...
#' df2 <- as.data.frame(df2 %>% dplyr::group_by(Symbol) %>% dplyr::group_by(num=n(),add=T) %>% dplyr::filter(num==length(table(df1$group))))
...
xAge <- function(list_vec, identity=c('Symbol','GeneID'), p.type=c("nominal","empirical"), seed=825, nperm=2000, p.adjust.method=c("BH","BY","bonferroni","holm","hochberg","hommel"), plot=FALSE, RData.location="http://galahad.well.ox.ac.uk/bigdata")
{
    ...
    sum(1/log2(df_PSG$Phylostrata +1))
    allindex <- 1:nrow(df_PSG)
    ...
    ls_df_gp <- lapply(1:length(list_vec), function(i){
		...
		if(p.type == 'nominal'){
			...
			if(0){
				...
				tstat <- signif(res$statistic, 2)
			
			}else{
				df1 <- data.frame(Phylostrata=x, data='observed', stringsAsFactors=F)
				df2 <- data.frame(Phylostrata=y, data='other', stringsAsFactors=F)
				df <- rbind(df1, df2)
				df$data <- as.factor(df$data)
				
				#coin::wilcox_test: asymptotic p-values, including in the presence of ties. That is, the conditional null distribution of the test statistic is used to obtain p-values (via an asymptotic approximation of the exact distribution)
				res <- coin::wilcox_test(Phylostrata ~ data, data=df, distribution="asymptotic")
				exp <- res@statistic@expectation
				obs <- res@statistic@linearstatistic
				
				pval_obs <- coin::pvalue(res)
				zscore <- coin::statistic(res, type="test")
				zscore <- signif(zscore, 3)
			}
			
			if(pval_obs < 0.05){
				pval_obs <- as.numeric(format(signif(pval_obs,2),scientific=TRUE))
			}else{
				pval_obs <- signif(pval_obs,3)
			}
			...
			if(0){
				...
				obs <- signif(sum(1/log2(df_PSG$Phylostrata[!is.na(ind)] +1)), 2)
			
				B <- nperm
				set.seed(seed)
				vec_exp <- sapply(1:B, function(i){
					ind <- sample(allindex, length(x))
					sum(1/log2(df_PSG$Phylostrata[ind] +1))
				})
				exp <- median(vec_exp)
				zscore <- (obs - exp) / stats::mad(vec_exp)
				zscore <- signif(zscore, 3)
			
				if(median(vec_exp) > obs){
					## the observed is less than the expected
					pval_obs <- sum(vec_exp <= obs)/B
				}else{
					## the observed is greater than the expected
				...
				gp_pdf <- gp + labs(x="Gene Evolutionary Age Cumulative Ranking", y=paste0("Probability density (null distribution)\nestimated based on ",B," permutations"), title=names(list_vec)[i], subtitle=paste0("Observed: ",obs,' (robust z-score: ',zscore,'; empirical p: ',pval_obs,')')) + theme(plot.title=element_text(hjust=0.5, size=10), plot.subtitle=element_text(hjust=0.5, size=8))
				##################################
		...
		df <- data.frame(name=names(list_vec)[i], num=length(x), observed=obs, expected=exp, ratio=log2(obs/exp), zscore=zscore, pval=pval_obs, stringsAsFactors=FALSE)
